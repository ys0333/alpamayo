...
cat > "$SUMMARY_CSV" <<'EOF'
layer,head,select_prefix,mean_minade_m,mean_e2e_ms,mean_diffusion_ms,mean_ratio_pct,status
EOF
...
for layer in $(seq "$LAYER_START" "$LAYER_END"); do
  for head in $(seq "$HEAD_START" "$HEAD_END"); do
    ...
    echo
    echo "=== Running head ablation layer=$layer head=$head ==="
    {
      printf 'COMMAND:'
      printf ' %q' "${cmd[@]}"
      printf '\n'
    ...
    if "${cmd[@]}" | tee -a "$raw_log"; then
      ...
      if [[ -n "$summary_line" ]]; then
        ...
        printf '%s,%s,"%s",%s,%s,%s,%s,%s\n' \
          "$layer" "$head" "$SELECT_PREFIX" "${mean_minade:-}" "${mean_e2e:-}" \
          "${mean_diffusion:-}" "${mean_ratio:-}" "ok" >> "$SUMMARY_CSV"
      else
        printf '%s,%s,"%s",,,,,%s\n' "$layer" "$head" "$SELECT_PREFIX" "missing_summary" >> "$SUMMARY_CSV"
      fi
    else
      printf '%s,%s,"%s",,,,,%s\n' "$layer" "$head" "$SELECT_PREFIX" "failed" >> "$SUMMARY_CSV"
    fi
  done
done
